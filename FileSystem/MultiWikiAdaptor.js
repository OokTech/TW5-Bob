...
if($tw.node) {
  ...
  */
  ...
  MultiWikiAdaptor.prototype.deleteWiki = function(data, cb) {
    ...
    if(MultiWikiAdaptor.existsListed(data.deleteWiki) && authorised) {
  ...
  */
  ...
  MultiWikiAdaptor.prototype.createWiki = function(data, cb) {
    ...
    if(authorised && quotasOk) {
      ...
      // the wiki name.
      const name = ($tw.settings.namespacedWikis === 'yes') ? $tw.ServerSide.GetWikiName((data.decoded.name || 'imaginaryPerson') + '/' + (data.wikiName || data.newWiki || 'NewWiki')) : $tw.ServerSide.GetWikiName(data.wikiName || data.newWiki);
      ...
      if(data.nodeWikiPath) {
        ...
        // name isn't in use
        if(MultiWikiAdaptor.existsListed(data.fromWiki)) {

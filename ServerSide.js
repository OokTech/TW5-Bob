...
ServerSide.prepareWiki = function (fullName, servePlugin, cache='yes') {
  ...
  if(typeof $tw.Bob.Wikis[fullName].modified === 'undefined' || $tw.Bob.Wikis[fullName].modified === true || typeof $tw.Bob.Wikis[fullName].cached !== 'string') {
    ...
    if(missingPlugins.length > 0) {
      $tw.syncapdaptor.loadPlugins(missingPlugins,$tw.config.pluginsPath,$tw.config.pluginsEnvVar, fullName);
    }
...
*/
ServerSide.GetWikiName = function(wikiName, count, wikiObj, fullName) {
  let updatedName;
  count = count || 0;
  wikiName = wikiName || ''
  if(wikiName.trim() === '') {
  ...
  if(nameParts.length > 1) {
    if(wikiObj[nameParts[0]]) {
      return $tw.ServerSide.GetWikiName(nameParts.slice(1).join('/'), count, wikiObj[nameParts[0]], fullName);
    } else {
      return fullName;
    }
  } else {
